...
new mwn().rawRequest({
  url: 'https://en.wikipedia.org/w/api.php?action=query&format=json&meta=languageinfo&formatversion=2&liprop=fallbacks&licode=*'
}).then(response => {
  ...
  const jsonString = JSON.stringify(fallbacks, null, 2)
      .replace(/\n    /g, ' ')
      .replace(/\n  \]/g, ' ]')
  fs.writeFileSync('../data/fallbacks.json', jsonString)
})
